				...
				<div class="nav-bottom">
					...
					<s:iterator value="list.{?#this.infoId==#bean.id}" var="beans" status="st">
								...
								<s:if test="#bean.id==5||#bean.id==1">
									...
									<s:else>
											<a href="queryInfoContent.do?id=${beans.id }&a=${bean.id }">${beans.title
										}</a>
									</s:else>
								</s:if>
								<s:elseif test="#bean.id==3&&#beans.title!='微信'">
									<a href="${beans.linkPath }" target="_blank"
										style="background: url(${beans.imgUrl }) no-repeat 0 2px; padding: 0 0 2px 29px;">${beans.title
										}</a>
								</s:elseif>
								<s:elseif test="#bean.id==3&&#beans.title=='微信'">
						...
						<s:if test="#bean.id==3">
								<li id="bdShare" class="bdsharebuttonbox">
									<a href="#" class="bds_more share" data-cmd="more"
										style="color: #aaf3b5; font-size: 14px; line-height: 20px">分享</a>
								</li>
						</s:if>
					</ul>
				</div>
