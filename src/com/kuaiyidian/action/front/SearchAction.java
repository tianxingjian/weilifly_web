...
import com.kuaiyidian.service.AdvertisementService;
import com.kuaiyidian.service.WeiliResearchesService;
import com.kuaiyidian.service.BrandService;
import com.kuaiyidian.service.DownloadHelpService;
import com.kuaiyidian.service.InfoContentService;
import com.kuaiyidian.service.KydReportService;
import com.kuaiyidian.service.NewspaperService;
import com.kuaiyidian.service.ProductCategoryService;
import com.kuaiyidian.service.QuestionService;
import com.kuaiyidian.service.RecruitService;
import com.kuaiyidian.service.UpdateProgramService;
import com.kuaiyidian.service.WebDistributeService;
import com.shove.Convert;

public class SearchAction extends BaseFrontAction {
	private WeiliResearchesService brandNewsService;  //品牌资讯
	...
	private UpdateProgramService updateProgramService;  //升级程序
	...
	private KydReportService kydReportService; //快易典报道
	...
	public String querySearch() throws Exception {
		...
		if(StringUtils.isNotBlank(keyWords)){
			...
			List<Map<String, Object>> bnList=brandNewsService.queryBrandNewsList(keyWords); //品牌资讯
			...
			List<Map<String, Object>> updateProgramList=updateProgramService.queryUpdateProgramList(keyWords);  //升级程序
			...
			List<Map<String, Object>> kydReportList=kydReportService.queryKydReportList(keyWords); //快易典报道
			...
			if(bnList!=null&&bnList.size()>0){
				for(Map<String,Object> map: bnList){
					...
					rMap.put("url", "weiliResearchesDetail.do?id="+map.get("id"));
					allResultList.add(rMap);
				}
			}
			...
			//升级程序
			if(updateProgramList!=null&&updateProgramList.size()>0){
				for(Map<String,Object> map: updateProgramList){
					totalNum ++ ;
					Map<String,String> rMap = new HashMap<String, String>();
					rMap.put("title", map.get("name")+"");
					rMap.put("url", "updateProgramIndex.do?a="+map.get("categoryId"));
					allResultList.add(rMap);
				}
			}
			//广告片
	...
	public void setBrandNewsService(WeiliResearchesService brandNewsService) {
		this.brandNewsService = brandNewsService;
	}
